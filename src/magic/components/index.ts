...
/* `./button` N'EST PLUS RÉEXPORTÉ, ET LE FICHIER RESTE.
   ...
   et `Button`. Deux composants du même nom, dont l'un imitait l'autre en
   CSS. `Button` délègue désormais au vendoré dès que `liquidGlass` est
...
/* SEPT AUTRES PORTES SE FERMENT POUR LA MÊME RAISON, ET LES SEPT FICHIERS
   ...
   côté Opale — `Card`, `Input`, `Checkbox`, `Select`,
   `Slider`, `Badge` et `Toggle`. Sept fois le même nom pour
   ...
   CHAQUE `X` DÉLÈGUE DÉSORMAIS À SON VENDORÉ QUAND `liquidGlass` EST VRAI,
   exactement comme `Button`. Le nom vendoré ne désignait donc plus un
   ...
   prop. `opale.tsx` importe les sept par leur chemin direct, donc fermer le
   ...
   `XProps`. `./toast` N'EST PAS DANS LA LISTE, et c'est délibéré : il
